        ...
        for name, fig in histograms.items():
            if model.logger is not None:
                if HAS_WANDB and wandb is not None:
                    ...
                    # For TensorBoard, log the figure directly
                    try:
                        model.logger.experiment.add_figure(f"{logname}/{name}", fig, model.global_step)
                    except Exception:
                        pass  # Skip if logger doesn't support add_figure
